document.addEventListener('DOMContentLoaded', () => {
    ...
    dbRef.on('value', (snapshot) => {
        ...
        alert("Veri yüklenirken bir hata oluştu. Konsolu kontrol edin.");
    });
    ...
    toggleButton.addEventListener('click', () => {
        ...
        if (editMode) {
            ...
            alert('Düzenleme Modu Kapatıldı. Son kaydedilen veriler gösteriliyor.');
        }
        renderScoreboard(editMode);
    });
    ...
    saveButton.addEventListener('click', () => {
        ...
        dbRef.set(scores)
            .then(() => {
                ...
                alert('Veriler başarıyla kaydedildi ve tüm ziyaretçiler için canlı güncellendi!');
                editMode = false;
            })
            .catch((error) => {
                alert('HATA: Veritabanına kayıt başarısız oldu: ' + error.message);
            });
    });
});
